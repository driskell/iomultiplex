...
module IOMultiplex
  ...
  class StringBuffer
    ...
    def peek
      ...
      i = 1
      while @buffer[i] && s.length + @buffer[i].length < 4096
        s << @buffer[i]
        i += 1
      end
      s
  ...
  class IOReactor
    ...
    def write_nonblock(data)
      @logger.debug 'write_nonblock', count: data.length
  ...
  class SSLIO < IOReactor
    ...
    def write_nonblock(data)
      @logger.debug 'SSL write_nonblock', count: data.length
  ...
  class Multiplexer
    ...
    def initialize(options={})
      if options[:logger]
        @logger = options[:logger].copy
      else
        @logger = Cabin::Channel.new
      end

      @id = options[:id] || object_id
      @logger['multiplexer'] = @id
